...
relevant parameter ranges. Clicking **Done** closes the app and returns
an updated `spectrum` object with the current Manual fit or current
Autofit result appended to its `fits` slot, which is why the call above
...
- the monoploid non-repetitive genome size (log10 Mbp).
...
2. **Parameter ranges** for k-mer coverage, variance factor, theta, and
   genome size. Theta and genome size use log10 scales, with genome size
   expressed as log10(Mbp). Tetmer searches within these bounds for the
...
Once you are satisfied with a fit obtained in Manual or Autofit mode, click
